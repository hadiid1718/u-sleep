 ...
 */
export const subscriptionWorkflow = serve(async (context) => {
  ...
  if (!user.expiresAt || user.status !== "active") {
    console.log(`Workflow skipped – user ${userId} has no active subscription.`);
    return;
  }
  ...
  for (const daysLeft of REMINDER_DAYS) {
    const sendAt = new Date(expiresAt.getTime() - daysLeft * 24 * 60 * 60 * 1000);

    // If the send date is already in the past, skip this reminder
    if (sendAt <= new Date()) {
      console.log(`Skipping ${daysLeft}-day reminder for ${user.email} – date already passed.`);
      continue;
    }
    ...
    const currentUser = await context.run(`check-status-${daysLeft}d`, async () => {
      const u = await User.findById(userId).select("subscription email").lean();
      return {
        email: u?.email,
    ...
    if (currentUser.status !== "active") {
      console.log(`Workflow stopped – user ${userId} subscription is no longer active.`);
      return;
    }

    // If expiresAt changed (renewed), stop this workflow — a new one should be triggered
    if (currentUser.expiresAt !== user.expiresAt) {
      console.log(`Workflow stopped – user ${userId} renewed their subscription.`);
      return;
    }

    // Send the reminder email
    await context.run(`send-email-${daysLeft}d`, async () => {
      const { subject, text, html } = buildReminderEmail(user.name, daysLeft, user.plan);
