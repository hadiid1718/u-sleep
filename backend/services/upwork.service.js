...
class UpworkService {
  ...
  transformUpworkJob(upworkJob) {
    return {
      ...
      clientInfo: {
        ...
        totalReviews:
          upworkJob.client?.reviews || upworkJob.buyer?.review_count,
        totalSpent:
          upworkJob.client?.total_spent || upworkJob.buyer?.total_spent,
        jobsPosted:
          upworkJob.client?.jobs_posted || upworkJob.buyer?.jobs_posted,
        paymentVerified:
          upworkJob.client?.payment_verified ||
          upworkJob.buyer?.payment_verified ||
          false,
        hireRate: upworkJob.client?.hire_rate || upworkJob.buyer?.hire_rate,
        country: upworkJob.client?.country || upworkJob.buyer?.country,
        totalHires:
          upworkJob.client?.total_hires || upworkJob.buyer?.total_hires,
      },
      isCached: true,
  ...
  async searchJobs(keywords, filters = {}) {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Upwork API Error: ${response.status} ${response.statusText}`
        );
      }
      ...
      return this.cacheEnabled ? await this.getCachedJobs(keywords) : [];
    }
  }
  ...
  buildSearchQuery(keywords, filters = {}) {
    ...
    if (filters.minClientRating)
      params.append('min_rating', filters.minClientRating);
   ...
   */
  async cacheJobs(jobs, _keywords) {
    try {
  ...
  jobMatchesBadCriteria(job, criteria) {
    ...
    if (lowerCriteria.includes('low budget') && job.budget?.amount < 500)
      return true;
    if (
    ...
    if (
      ...
      return true;
    if (lowerCriteria.includes('too many proposals') && job.proposalsCount > 50)
      return true;
    if (
      lowerCriteria.includes('unverified client') &&
      job.clientInfo?.jobsPosted < 5
    )
      return true;
