...
export const getComparisons = async (req, res, next) => {
  try {
    const comparisons = await Comparison.find({ isActive: true }).sort({
      order: 1,
    });
...
export const updateComparison = async (req, res, next) => {
  try {
    ...
    const comparison = await Comparison.findByIdAndUpdate(id, updates, {
      new: true,
      runValidators: true,
    });
...
export const deleteComparison = async (req, res, next) => {
  try {
    ...
    res
      .status(200)
      .json({ success: true, message: 'Comparison deleted successfully' });
  } catch (error) {
    next(error);
  }
};
  ...
  try {
    ...
    if (count > 0) {
      const error = new Error(
        'Comparisons already exist. Delete them first to re-seed.'
      );
      error.statusCode = 400;
      throw error;
    }
    ...
    const defaults = [
      ...
      },
      {
        feature: 'Learning',
        uSleep: '$0',
        human: '$0+ per freelance call',
        order: 2,
      },
      {
        feature: 'Proposal writing',
        uSleep: '$0',
        human: '$0+ per opportunity',
        order: 3,
      },
      {
        feature: 'Client outreach',
        uSleep: '$0+ per opportunity',
        human: '$0',
        order: 4,
      },
      {
        feature: 'Overall work',
        uSleep: '$0+ per freelance work',
        human: '$45+ per hour',
        order: 5,
      },
      {
        feature: 'Work anxiety',
        uSleep: '$0 resolved',
        human: 'Panic attacks',
        order: 6,
      },
      {
        feature: 'Upwork commission',
        uSleep: '$450 Dollars',
        human: '$450+ Dollars',
        order: 7,
      },
    ...
    res.status(201).json({
      success: true,
      data: comparisons,
      message: 'Default comparisons seeded successfully',
    });
  } catch (error) {
    next(error);
  }
