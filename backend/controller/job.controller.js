...
export const searchJobs = async (req, res, next) => {
    try {
        ...
        upworkService.searchJobs(keywords, filters)
            .then(async (jobs) => {
                // Apply bad job filters
                let filteredJobs = upworkService.applyBadJobFilters(jobs, user.jobPreferences?.badJobCriteria);

                // Apply rate matching
                if (user.jobPreferences?.rateType) {
                    const rate = user.jobPreferences?.rateType === 'hourly' ?
...
export const searchJobsWithAIAnalysis = async (req, res, next) => {
    try {
        ...
        // Sort by match score
        jobsWithScores.sort((a, b) => b.aiAnalysis.matchScore - a.aiAnalysis.matchScore);
...
const calculateMatchScore = (job, user) => {
    ...
    } else if (user.jobPreferences?.rateType === 'hourly' && job.budgetType === 'hourly') {
...
const extractGreenFlags = (job) => {
    ...
    if ((job.clientInfo?.rating || 0) >= 4.5) flags.push(`Top Rated: ${job.clientInfo?.rating}`);
    if (job.proposalsCount <= 10) flags.push('Low Competition');
    if (job.description && job.description.length > 300) flags.push('Clear Requirements');
...
const extractRedFlags = (job) => {
    ...
    if (job.description && job.description.length < 100) flags.push('Vague Description');
...
const generateReasoning = (job, user, score) => {
    return `This job matches your profile with a ${score}/100 score. The budget aligns with your rates, ` +
