...
import { getErrorMessage } from '../../services/core/apiClient';
...
const ScheduleDemo = () => {
  ...
  const fetchAvailableDates = async () => {
    try {
      const response = await demoAPI.getAvailableDates();
  ...
  const fetchAvailableTimes = async (date) => {
    try {
      setLoading(true);
      const response = await demoAPI.getAvailableTimes(date);
  ...
  const handleScheduleDemo = async () => {
    ...
    try {
      const response = await demoAPI.scheduleDemo({
        email,
        name,
        company,
        phone,
