...
// Main Job Results Component
const JobResults = () => {
  const { jobs, formData, resetForm } = useContext(AppContext);
    ...
    <div className="max-w-6xl mx-auto">
      ...
      <div className="text-center mb-8">
        <h2 className="text-4xl font-bold text-white mb-4">
          Found <span className="text-lime-400">{jobs.length}</span> Quality Jobs
        </h2>
        <p className="text-gray-400">Sorted by AI quality score (highest first)</p>
      </div>

      {/* Search Criteria Summary */}
      <div className="bg-gray-800 p-6 rounded-lg mb-8">
        <h3 className="text-xl font-bold text-white mb-4">Your Search Criteria:</h3>
        <div className="grid md:grid-cols-2 gap-4 text-gray-300">
          <div>
            <span className="text-lime-400 font-semibold">Keywords:</span>{' '}
            {formData.keywords.join(', ')}
          </div>
          <div>
            <span className="text-lime-400 font-semibold">Rates:</span>{' '}
            ${formData.hourlyRate}/hr | ${formData.fixedRate}/project
          </div>
          <div>
            <span className="text-lime-400 font-semibold">Account:</span>{' '}
            {formData.accountType}
          </div>
          <div>
            <span className="text-lime-400 font-semibold">Filters:</span>{' '}
            {formData.badJobCriteria.length} criteria applied
          </div>
        </div>
      </div>
      ...
      <button
        onClick={resetForm}
        className="w-full bg-lime-400 text-black py-3 rounded-lg font-bold hover:bg-lime-300 transition-colors"
      >
        Search Again
      </button>
    </div>
